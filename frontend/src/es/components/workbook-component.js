...
export default class WorkbookComponent extends Good<P, {}> {
  ...
  detectPressAction (isRight = false) {
    const { mode, keyControl } = this.props;
  ...
  startDrawMarker (startX, startY) {
    ...
    const offsetX = -this.currentPage.pagePosition.x;
    const offsetY = -this.currentPage.pagePosition.y;
    const marker = this.currentPage.newMarker((startX + offsetX) / scale, (startY + offsetY) / scale, this.props.thickness);
  ...
  startDrag (startX, startY, isRight = false) {
    const drag = this.detectDragAction(isRight);
  ...
  mousePosition (e:MouseEvent) {
    const x = e.pageX - this.workspace.offsetLeft - 100;
    const y = e.pageY - this.workspace.offsetTop - 40;
    return { x, y };
  }
  ...
  render () {
    ...
    const { workbook, mode, size, dataURL, thickness, sheetVisibility, scale, workbookState } = this.props;
